...
class Custom(BaseWatermark):
    ...
    def generate_watermarked_code(self, prompt: str, *args, **kwargs):
        ...
        encoded_prompt = self.config.generation_tokenizer(prompt, return_tensors="pt", add_special_tokens=True).to(self.config.device)
        # generate watermarked code
        encoded_watermarked_code = generate_with_watermark(**encoded_prompt)
        # decode
        watermarked_code = self.config.generation_tokenizer.batch_decode(encoded_watermarked_code, skip_special_tokens=True)[0]
